...
public class SwaggerJsonConfiguration {
	...
	@Bean
	public SwaggerJson swaggerConfiguration(@Value("${swagger.json.location}") String location, ResourceLoader loader)
			throws IOException {

		InputStream istream = getResource(location, loader);
		if (istream == null) {
			throw new FileNotFoundException("Scope configuration file " + location + " not found.");
		}
	...
	private InputStream getResource(String location, ResourceLoader loader) {
		...
		try {
			...
			System.out.println("Searching for swagger JSON. Not found in " + location);
		}
		...
		if (istream == null) {
			...
			for (String scheme : schemes) {
				try {
					...
					System.out.println("Searching for swagger JSON. Not found in " + scheme);
				}
				if (istream != null) {
					return istream;
				}
